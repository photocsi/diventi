...
$id_cliente4 = $_SESSION['id_cliente4'];
if ($id_cliente4 == "NULL") {
  $id_cliente4 = $_COOKIE['id_operatore'];
}
...
if(isset($cartella_scelta4)){
  foreach ($cartella_scelta4 as $cartella4) {
    $seleziona_foto = $conn->prepare("SELECT *  FROM `$id_album` WHERE (sotto_cartella= :cartella_scelta) ORDER BY data ASC;");
    $seleziona_foto->bindparam(":cartella_scelta",  $cartella4);
  ...
  $selezione = prendi_preferiti($id_album, $id_cliente4);
