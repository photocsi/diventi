...
session_start();
        ...
        $id_cliente2 = $_SESSION['id_cliente2'];
        if ($id_cliente2 == "NULL") {
          $id_cliente2 = $_COOKIE['id_operatore'];
        }
        ...
        if(isset($cartella_scelta2)){
          foreach ($cartella_scelta2 as $cartella2) {
            $seleziona_foto = $conn->prepare("SELECT *  FROM `$id_album` WHERE (sotto_cartella= :cartella_scelta) ORDER BY data ASC;");
            $seleziona_foto->bindparam(":cartella_scelta",  $cartella2);
          ...
          $selezione = prendi_preferiti($id_album, $id_cliente2);
